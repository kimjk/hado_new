...
* 프로그램명	 : SOent_Violate_43_proc.jsp																				   
* 프로그램설명	: 수급사업자 > 조사결과분석 > 원재료가격 인상을 이유로 하도급대금 인상 요구 여부  
* 프로그램버전	: 1.0.1																				                        
		...
		if( !tt.equals("start") ) {
			if (currentYear>=2010 ) { 
				if( !session.getAttribute("wgb").equals("") ) {
					sSQLs="SELECT a.Oent_Type,b.Common_NM,a.Field01,a.Field02,a.Field03,a.Field04 \n";
					sSQLs+="FROM (SELECT "+currentOent+".Oent_Type, \n";
				} else {
					sSQLs="SELECT Oent_GB Oent_Type,'' Common_NM,Field01,Field02,Field03,Field04 \n";
				...
				if( currentYear>=2012 ) {
					...
					sSQLs+="SUM(CASE HADO_TB_SOent_Answer.a WHEN '1' THEN 1 ELSE 0 END) Field02, \n";
					sSQLs+="SUM(CASE HADO_TB_SOent_Answer.b WHEN '1' THEN 1 ELSE 0 END) Field03, \n";
					sSQLs+="SUM(CASE HADO_TB_SOent_Answer.c WHEN '1' THEN 1 ELSE 0 END) Field04 \n";
					sSQLs+="FROM "+currentOent+",HADO_TB_Subcon_"+currentYear+",HADO_TB_SOent_Answer \n";
					sSQLs+="WHERE HADO_TB_Subcon_"+currentYear+".Oent_GB=HADO_TB_SOent_Answer.Oent_GB \n";
				}
				...
				*/
				...
				if( currentYear>=2012 ) {
					...
					sSQLs+="AND HADO_TB_Subcon_"+currentYear+".Child_Mng_No=HADO_TB_SOent_Answer.Mng_No \n";
					sSQLs+="AND HADO_TB_Subcon_"+currentYear+".Current_Year=HADO_TB_SOent_Answer.Current_Year \n";
				}
				if( session.getAttribute("wgb").equals("1") ) {
					if( currentYear>=2012 ) {
						sSQLs+="AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=4 \n";
					} else {
						sSQLs+="AND HADO_TB_SOent_Answer.soent_q_cd=19 AND HADO_TB_SOent_Answer.soent_q_gb=4 \n";
					}
				} else if( session.getAttribute("wgb").equals("2") ) {
					if( currentYear>=2015 ) {
					sSQLs+="		AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=13 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=4 \n";
					} else {
						sSQLs+="AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=12 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=4 \n";
					}
				} else if( session.getAttribute("wgb").equals("3") ) {
					if( currentYear>=2012 ) {
						sSQLs+="AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=4 \n";
					} else {
						sSQLs+="AND HADO_TB_SOent_Answer.soent_q_cd=17 AND HADO_TB_SOent_Answer.soent_q_gb=4 \n";
					}
				} else {
					if( currentYear>=2015 ) {
					sSQLs+="		AND ( (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=4 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='1') \n";
					sSQLs+="			OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=13 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=4 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='2') \n";
					sSQLs+="			OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=4 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='3') ) \n";
					} else if( currentYear>=2012 ) {
						sSQLs+="AND ( (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=4 AND HADO_TB_Subcon_"+currentYear+".Oent_GB='1') \n";
						sSQLs+="	OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=12 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=4 AND HADO_TB_Subcon_"+currentYear+".Oent_GB='2') \n";
						sSQLs+="	OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=4 AND HADO_TB_Subcon_"+currentYear+".Oent_GB='3') ) \n";
					} else {
						sSQLs+="AND ((HADO_TB_SOent_Answer.soent_q_cd=19 AND HADO_TB_SOent_Answer.soent_q_gb=4  AND HADO_TB_Subcon_"+currentYear+".oent_gb='1') \n";
						sSQLs+="OR ( HADO_TB_SOent_Answer.soent_q_cd=17  AND  HADO_TB_SOent_Answer.soent_q_gb=4  AND HADO_TB_Subcon_"+currentYear+".oent_gb='3')) \n";
					}
				}
				...
				if( currentYear>=2012 ) {
					...
					sSQLs+="AND HADO_TB_Subcon_"+currentYear+".sent_no=HADO_TB_SOent_Answer.sent_no \n";
				}
				...
				if( !session.getAttribute("wgb").equals("") ) {
					sSQLs="SELECT a.Oent_Type,b.Common_NM,a.Field01,a.Field02,a.Field03,a.Field04 \n";
					sSQLs+="FROM (SELECT HADO_TB_Oent.Oent_Type, \n";
				} else {
					sSQLs="SELECT Oent_GB Oent_Type,'' Common_NM,Field01,Field02,Field03,Field04 \n";
					sSQLs+="FROM (SELECT HADO_TB_Subcon_"+currentYear+".Oent_GB, \n";
				}
				...
				sSQLs+="SUM(CASE HADO_TB_SOent_Answer.b WHEN '1' THEN 1 ELSE 0 END) Field03, \n";
				sSQLs+="SUM(CASE HADO_TB_SOent_Answer.c WHEN '1' THEN 1 ELSE 0 END) Field04 \n";
				sSQLs+="FROM HADO_TB_Oent,HADO_TB_Subcon_"+currentYear+",HADO_TB_SOent_Answer \n";
				sSQLs+="WHERE HADO_TB_Subcon_"+currentYear+".Oent_GB=HADO_TB_SOent_Answer.Oent_GB \n";
				...
				sSQLs+="AND HADO_TB_Subcon_"+currentYear+".Current_Year=HADO_TB_SOent_Answer.Current_Year \n";
				if( session.getAttribute("wgb").equals("1") ) {
					sSQLs+="AND HADO_TB_SOent_Answer.soent_q_cd=19 AND HADO_TB_SOent_Answer.soent_q_gb=2 \n";
				} else if( session.getAttribute("wgb").equals("3") ) {
					sSQLs+="AND HADO_TB_SOent_Answer.soent_q_cd=17 AND HADO_TB_SOent_Answer.soent_q_gb=2 \n";
				} else {
					sSQLs+="AND ((HADO_TB_SOent_Answer.soent_q_cd=19 AND HADO_TB_SOent_Answer.soent_q_gb=2  AND HADO_TB_Subcon_"+currentYear+".oent_gb='1') \n";
					sSQLs+="OR ( HADO_TB_SOent_Answer.soent_q_cd=17  AND  HADO_TB_SOent_Answer.soent_q_gb=2  AND HADO_TB_Subcon_"+currentYear+".oent_gb='3')) \n";
				}
				sSQLs+="AND HADO_TB_Subcon_"+currentYear+".sent_no=HADO_TB_SOent_Answer.sent_no \n";
				...
				sSQLs+="AND HADO_TB_Subcon_"+currentYear+".Current_Year='"+currentYear+"' \n";
				if( !session.getAttribute("wgb").equals("") ) {
					sSQLs+="GROUP BY HADO_TB_Oent.Oent_Type) a \n";
					sSQLs+="LEFT JOIN Common_CD b \n";
					sSQLs+="ON b.Addon_GB='"+session.getAttribute("wgb")+"' \n";
					sSQLs+="AND a.Oent_Type=b.Common_cd \n";
					sSQLs+="ORDER BY oent_type \n";
...
content+="  </tr>";
content+="	<tr>";
content+="		<th>당사가 원사업자에게 직접 요청</th>";
content+="		<th>당사가 소속된 중소기업협동조합을 통해 요청</th>";
content+="		<th>요청하지 않았음</th>";
content+="  </tr>";
