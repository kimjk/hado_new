...
* 프로그램명	 : SOent_Violate_60_proc.jsp																				   
* 프로그램설명	: 수급사업자 > 조사결과분석 > 하도급대금 감액 사유 통보 방법   
* 프로그램버전	: 1.0.1																				                        
	...
	try {
		...
		if( !tt.equals("start") ) {
			...
			if( !session.getAttribute("wgb").equals("") ) {
				sSQLs="SELECT a.Oent_Type,b.Common_NM,a.Field01,a.Field02,a.Field03,a.Field04,a.Field05,a.Field06 FROM ( \n";
				sSQLs+="	SELECT "+currentOent+".Oent_Type, \n";
			} else {
				sSQLs="SELECT Oent_GB Oent_Type,'' Common_NM,Field01,Field02,Field03,Field04,Field05,Field06 FROM ( \n";
			...
			*/
			...
			if( session.getAttribute("wgb").equals("1") ) {
					sSQLs+="	AND (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=4 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=3 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='1') \n";
			} else if( session.getAttribute("wgb").equals("2") ) {
					sSQLs+="	AND (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=4 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=3 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='2') \n";
			} else if( session.getAttribute("wgb").equals("3") ) {
					sSQLs+="	AND (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=4 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=3 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='3') \n";
			} else {
					sSQLs+="	AND ( (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=4 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=3 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='1') \n";
					sSQLs+="	OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=4 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=3 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='2') \n";
					sSQLs+="	OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=4 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=3 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='3') ) \n";
			}
...
content+="		<th colspan='5'>하도급대금 감액 사유 통보 방법 (중복응답)</th>";
content+="  </tr>";
content+="	<tr>";
content+="		<th>감액하기 전, 구두 설명</th>";
content+="		<th>대금 지급 시, 구두 설명</th>";
content+="		<th>감액하기 전, 서면 통보</th>";
content+="		<th>대금 지급 시, 서면 통보</th>";
content+="		<th>알려주지 않았음</th>";
