	...
	try {
		...
		if( !tt.equals("start") ) {
			...
			sSQLs = "SELECT SENT_TYPE,COMMON_NM, " +
					"SUM(CASE WHEN AR1='1' OR AR2='1' OR AR3='1' THEN 1 ELSE 0 END) FIELD01, " +
					"SUM(CASE AR1 WHEN '1' THEN 1 ELSE 0 END) FIELD02, " +
					"SUM(CASE AR2 WHEN '1' THEN 1 ELSE 0 END) FIELD03, " +
					"SUM(CASE AR3 WHEN '1' THEN 1 ELSE 0 END) FIELD04 " +
					"FROM ( " +
					"SELECT E.SENT_TYPE,F.COMMON_NM,E.ANSWER1,E.AR1,E.AR2,E.AR3 " +
					"FROM (SELECT C.*,(CASE D.INDEX_VALUE WHEN '1' THEN '1' ELSE '0' END) AR1, " +
					"(CASE INDEX_VALUE WHEN '2' THEN '1' ELSE '0' END) AR2, " + 
					"(CASE INDEX_VALUE WHEN '3' THEN '1' ELSE '0' END) AR3 FROM ( " +
					...
					") A LEFT JOIN ( " +
					"SELECT MNG_NO,CURRENT_YEAR,OENT_GB,ANSWER1 FROM ( " +
					"SELECT AA.*,BB.ANSWER2 FROM ( " +
					"SELECT MNG_NO,CURRENT_YEAR,OENT_GB,'1' ANSWER1 FROM HADO_TB_SENT_NANSWER  ";

			if( currentYear == 2010 ) {
				sSQLs += "WHERE SENT_Q_CD=16 AND SENT_Q_GB=4 AND INDEX_VALUE='1' GROUP BY MNG_NO,CURRENT_YEAR,OENT_GB ) AA ";
			} else {
				sSQLs += "WHERE SENT_Q_CD=14 AND SENT_Q_GB=2 AND INDEX_VALUE='1' GROUP BY MNG_NO,CURRENT_YEAR,OENT_GB ) AA ";
			}
			
			sSQLs += "LEFT JOIN ( " +
					"SELECT MNG_NO,CURRENT_YEAR,OENT_GB,'1' ANSWER2 FROM HADO_TB_SENT_NANSWER ";
			
			if( currentYear == 2010 ) {
				sSQLs += "WHERE SENT_Q_CD=16 AND SENT_Q_GB=2 AND INDEX_VALUE='1' GROUP BY MNG_NO,CURRENT_YEAR,OENT_GB) BB ";
			} else { 
				sSQLs += "WHERE SENT_Q_CD=14 AND SENT_Q_GB=1 AND INDEX_VALUE='1' GROUP BY MNG_NO,CURRENT_YEAR,OENT_GB) BB ";
			}

			sSQLs += "ON AA.MNG_NO=BB.MNG_NO AND AA.CURRENT_YEAR=BB.CURRENT_YEAR AND AA.OENT_GB=BB.OENT_GB " +
					") DDD WHERE ANSWER2='1') B " +
					"ON A.MNG_NO=B.MNG_NO AND A.CURRENT_YEAR=B.CURRENT_YEAR AND A.OENT_GB=B.OENT_GB) C " +
					"LEFT JOIN " +
					"(SELECT MNG_NO,CURRENT_YEAR,OENT_GB,INDEX_VALUE FROM HADO_TB_SENT_NANSWER ";
			
			if( currentYear == 2010 ) {
				sSQLs += "WHERE SENT_Q_CD=16 AND SENT_Q_GB=7) D ";
			} else {
				sSQLs += "WHERE SENT_Q_CD=14 AND SENT_Q_GB=4) D ";
			}
			...
			nLoop = 1;
			while( rs.next() ) {
				arrData[nLoop][5] = rs.getString("sent_Type");
				arrData[nLoop][6] = new String( StringUtil.checkNull(rs.getString("Common_NM")).trim().getBytes("ISO8859-1"), "utf-8" );
...
content+="		<th colspan=3>수용 여부</th>";
content+="	</tr>";
content+="	<tr>";
content+="		<th>전부 수용</th>";
content+="		<th>일부 수용</th>";
content+="		<th>수용하지 않음</th>";
content+="	</tr>";
<%if( !tt.equals("start") ) {%>
...
content+="	<tr onMouseOver='tbRow_Over(this)' onMouseOut='tbRow_Out(this)'>";
	<%for(int i=2; i<=4; i++) {%>
	content+="		<td><%if( arrSum[1] > 0F) {%><%=formater.format(arrSum[i] / arrSum[1] * 100F)%><%} else { out.print("0.0"); }%>%</td>";
	<%}%>
content+="	</tr>";

<%for(int ni=1; ni<=nLoop; ni++) {%>
content+="	<tr onMouseOver='tbRow_Over(this)' onMouseOut='tbRow_Out(this)'>";
content+="		<th rowspan=2><%=arrData[ni][6]%></th>";
content+="		<td rowspan=2><%=formater2.format(Float.parseFloat(arrData[ni][1]))%></td>";
	<%for(int i=2; i<=4; i++) {%>
...
content+="	<tr onMouseOver='tbRow_Over(this)' onMouseOut='tbRow_Out(this)'>";
	<%for(int i=2; i<=4; i++) {%>
content+="		<td><%if( Float.parseFloat(arrData[ni][1]) > 0F) {%><%=formater.format(Float.parseFloat(arrData[ni][i]) / Float.parseFloat(arrData[ni][1]) * 100F)%><%} else { out.print("0.0"); }%>%</td>";
	<%}%>
content+="	</tr>";
<%}%>
<%}%>
