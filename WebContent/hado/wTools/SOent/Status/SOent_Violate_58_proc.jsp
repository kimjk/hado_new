...
* 프로그램명	 : SOent_Violate_58_proc.jsp																				   
* 프로그램설명	: 수급사업자 > 조사결과분석 > 위탁 취소 및 변경 사유  
* 프로그램버전	: 1.0.1																				                        
		...
		if( !tt.equals("start") ) {
			...
			*/
			...
			if( session.getAttribute("wgb").equals("1") ) {
					sSQLs+="	AND (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=3 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=2 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='1') \n";
			} else if( session.getAttribute("wgb").equals("2") ) {
					sSQLs+="	AND (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=3 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=2 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='2') \n";
			} else if( session.getAttribute("wgb").equals("3") ) {
					sSQLs+="	AND (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=3 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=2 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='3') \n";
			} else {
					sSQLs+="	AND ( (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=3 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=2 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='1') \n";
					sSQLs+="	OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=3 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=2 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='2') \n";
					sSQLs+="	OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=3 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=2 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='3') ) \n";
			}
...
content+="		<th colspan='6'>위탁 취소 및 변경 사유 (중복응답)</th>";
content+="  </tr>";
content+="	<tr>";
content+="		<th>원사업자도 발주자나 거래처로부터 주문을 취소</th>";
content+="		<th>부도 등으로 상호 정산하기로 합의</th>";
content+="		<th>원사업자 제품의 판매부진</th>";
content+="		<th>납품하는 제품의 사양 변경 또는 모델단종</th>";
content+="		<th>납품기일 등을 지키지 못함</th>";
content+="		<th>귀책사유가 없음에도 원사업자가 일방적으로 제조위탁을 취소하거나 변경</th>";
