...
* 프로그램명	 : SOent_Violate_64_proc.jsp																				   
* 프로그램설명	: 수급사업자 > 조사결과분석 > 대금인상 요청하지 않은 사유 
* 프로그램버전	: 1.0.1																				                        
...
/*---------------------------------------- Variable Difinition ----------------------------------------*/
	...
	String[][] arrData=new String[31][10];
...
/*=================================== Record Selection Processing =====================================*/
	...
	try {
		...
		if( !tt.equals("start") ) {
			...
			sSQLs+="SUM(CASE HADO_TB_SOent_Answer_"+currentYear+".c WHEN '1' THEN 1 ELSE 0 END) AS Field06, \n";
			sSQLs+="SUM(CASE HADO_TB_SOent_Answer_"+currentYear+".d WHEN '1' THEN 1 ELSE 0 END) AS Field07 \n";
			...
			if( session.getAttribute("wgb").equals("1") ) {
				sSQLs+="AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=7 \n";
			} else if( session.getAttribute("wgb").equals("2") ) {
				sSQLs+="		AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=13 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=7 \n";
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=7 \n";
			} else {
				sSQLs+="		AND ( (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=7 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='1') \n";
				sSQLs+="			OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=13 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=7 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='2') \n";
				sSQLs+="			OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=7 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='3') ) \n";
			}
...
content+="		<th colspan='6'>사유</th>";
content+="  </tr>";
content+="	<tr>";
content+="		<th>원재료 가격 인상폭 작아 대금 조정 불요</th>";
content+="		<th>계약시기 조정</th>";
content+="		<th>조정가능성 없음</th>";
content+="		<th>다음 계약에 반영</th>";
content+="		<th>거래 단절 등 원사업자 보복 우려</th>";
content+="		<th>기타</th>";
content+="  </tr>";
