...
* 프로그램명	 : SOent_Violate_44_proc.jsp																				   
* 프로그램설명	: 수급사업자 > 조사결과분석 > 원재료 인상에 따른 하도급대금 조정 협의 여부
* 프로그램버전	: 1.0.1																				                        
			...
			if (currentYear>=2010 ) { 
				...
				*/
				...
				if( session.getAttribute("wgb").equals("1") ) {
					if( currentYear>=2012 ) {
						sSQLs+="AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=5 \n";
					} else {
						sSQLs+="AND HADO_TB_SOent_Answer.soent_q_cd=19 AND HADO_TB_SOent_Answer.soent_q_gb=5 \n";
					}
					...
					sSQLs+="		AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=13 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=5 \n";
						...
						sSQLs+="AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=12 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=5 \n";
					}
				} else if( session.getAttribute("wgb").equals("3") ) {
					if( currentYear>=2012 ) {
						sSQLs+="AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=5 \n";
					} else {
						sSQLs+="AND HADO_TB_SOent_Answer.soent_q_cd=17 AND HADO_TB_SOent_Answer.soent_q_gb=5 \n";
					}
				} else {
					if( currentYear>=2015 ) {
					sSQLs+="		AND ( (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=5 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='1') \n";
					sSQLs+="			OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=13 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=5 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='2') \n";
					sSQLs+="			OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=5 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='3') ) \n";
					} else if( currentYear>=2012 ) {
						sSQLs+="AND ( (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=5 AND HADO_TB_Subcon_"+currentYear+".Oent_GB='1') \n";
						sSQLs+="	OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=12 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=5 AND HADO_TB_Subcon_"+currentYear+".Oent_GB='2') \n";
						sSQLs+="	OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=5 AND HADO_TB_Subcon_"+currentYear+".Oent_GB='3') ) \n";
					} else {
						sSQLs+="AND ((HADO_TB_SOent_Answer.soent_q_cd=19 AND HADO_TB_SOent_Answer.soent_q_gb=5  AND HADO_TB_Subcon_"+currentYear+".oent_gb='1') \n";
						sSQLs+="OR ( HADO_TB_SOent_Answer.soent_q_cd=17  AND  HADO_TB_SOent_Answer.soent_q_gb=5  AND HADO_TB_Subcon_"+currentYear+".oent_gb='3')) \n";
					}
				}
				...
				if( !session.getAttribute("wgb").equals("") ) {
					sSQLs="SELECT a.Oent_Type,b.Common_NM,a.Field01,a.Field02,a.Field03 \n";
					sSQLs+="FROM (SELECT HADO_TB_Oent.Oent_Type, \n";
				} else {
					sSQLs="SELECT Oent_GB Oent_Type,'' Common_NM,Field01,Field02,Field03 \n";
					sSQLs+="FROM (SELECT HADO_TB_Subcon_"+currentYear+".Oent_GB, \n";
				}
				sSQLs+="NVL(COUNT(HADO_TB_Subcon_"+currentYear+".Mng_No),0) Field01, \n";
				sSQLs+="SUM(CASE WHEN (HADO_TB_SOent_Answer.a='1' OR HADO_TB_SOent_Answer.b='1' OR HADO_TB_SOent_Answer.c='1' OR HADO_TB_SOent_Answer.d='1') THEN 1 ELSE 0 END) Field02, \n";
				sSQLs+="SUM(CASE HADO_TB_SOent_Answer.e WHEN '1' THEN 1 ELSE 0 END) Field03 \n";
...
content+="		<th colspan='5'>하도급대금 조정 협의 여부</th>";
content+="  </tr>";
content+="	<tr>";
content+="		<th>5일 이내</th>";
content+="		<th>6~10일 이내</th>";
content+="		<th>11~15일 이내</th>";
content+="		<th>15일 이상</th>";
content+="		<th>협의에 응해 주지 않았음</th>";
content+="  </tr>";
