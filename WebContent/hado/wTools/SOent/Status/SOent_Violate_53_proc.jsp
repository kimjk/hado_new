...
* 프로그램명	 : SOent_Violate_53_proc.jsp																				   
* 프로그램설명	: 수급사업자 > 조사결과분석 > 부당특약 관련  
* 프로그램버전	: 1.0.1																				                        
		...
		if( !tt.equals("start") ) {
			...
			*/
			...
			if( session.getAttribute("wgb").equals("1") ) {
					sSQLs+="	AND (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=12 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=1 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='1') \n";
			} else if( session.getAttribute("wgb").equals("2") ) {
				if( currentYear >= 2015 ) {
						sSQLs+="	AND (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=16 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=1 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='2') \n";
				} else {
					...
					sSQLs+="	AND (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=12 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=1 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='3') \n";
			} else {
				if( currentYear >= 2015 ) {
					sSQLs+="	AND ( (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=12 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=1 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='1') \n";
					sSQLs+="	OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=16 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=1 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='2') \n";
...
content+="		<th colspan='10'>부당특약 내용 (중복응답)</th>";
content+="  </tr>";
content+="	<tr>";
content+="		<th>계약서 등에 기재되지 않은 사항을 요구</th>";
content+="		<th>민원처리, 산업재해 등과 관련된 비용</th>";
content+="		<th>입찰내역에 없는 사항을 요구</th>";
content+="		<th>인·허가, 환경관리 또는 품질관리 등의 의무를 부담</th>";
content+="		<th>발주자가 설계나 작업내용을 변경하여 발생하는 비용</th>";
content+="		<th>재작업, 추가작업 또는 보수작업이 귀사에게 책임이 없는 사유로 이루어진 경우</th>";
content+="		<th>하자담보책임 또는 손해배상책임 부분을 귀사가 부담</th>";
content+="		<th>천재지변 등 계약시점에 예측할 수 없는 사항으로 발생하는 비용</th>";
content+="		<th>해당 하도급거래의 특성을 고려하지 아니한 채 간접비의 인정범위를 일률적으로 제한하는 조항</th>";
content+="		<th>원재료의 가격이 상승한 경우에도 귀사는 대금조정을 신청하지 아니한다는 조항</th>";
