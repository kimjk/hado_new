...
/*=================================== Record Selection Processing =====================================*/
	...
	try {
		...
		sSQLs ="SELECT SOent_Q_CD,SOent_Q_GB,SOent_Q_CDNM,Money_GB,IDentity_Q_CD,IDentity_Q_GB \n";
		...
		while( rs.next() ) {
			...
			if( tmp_MoneyGB.equals("1") ) {
				arrQCD1.add(rs.getString("SOent_Q_CD"));
				arrQGB1.add(rs.getString("SOent_Q_GB"));
				arrCDNM1.add(new String(rs.getString("SOent_Q_CDNM").trim().getBytes("ISO8859-1"), "utf-8"));
				arrMoneyGB1.add(tmp_MoneyGB);
				arrIDT_QCD1.add(rs.getString("Identity_Q_CD"));
				arrIDT_QGB1.add(rs.getString("Identity_Q_GB"));
			} else {
				arrQCD2.add(rs.getString("SOent_Q_CD"));
				arrQGB2.add(rs.getString("SOent_Q_GB"));
				arrCDNM2.add(new String(rs.getString("SOent_Q_CDNM").trim().getBytes("ISO8859-1"), "utf-8"));
		...
		for(int i=0; i<arrQCD1.size(); i++) {
			...
			if( tmp_Duble.equals("T") ) {
				...
				sSurveySQL+="CASE SUM(CASE S_A.SOent_Q_CD WHEN "+prev_QCD+" THEN CASE WHEN S_A.SOent_Q_GB IN ("+prev_QGB+","+arrQGB1.get(i)+") THEN 1 ELSE 0 END ELSE 0 END) WHEN 2 THEN 1 WHEN 1 THEN 1 ELSE 0 END cnt_"+prev_QCD+prev_QGB;
				tmp_Duble="F";
			} else if( nWriteCnt==2 ) {
				if( !sSurveySQL.equals("") ) { sSurveySQL+=", \n"; }
				sSurveySQL+="SUM(CASE S_A.SOent_Q_CD WHEN "+prev_QCD+" THEN CASE S_A.SOent_Q_GB WHEN "+prev_QGB+" THEN 1 ELSE 0 END ELSE 0 END) cnt_"+prev_QCD+prev_QGB;
				nWriteCnt=1;
			}
		...
		if( tmp_Duble.equals("T") ) {
			if( !sSurveySQL.equals("") ) { sSurveySQL+=", \n"; }
			sSurveySQL+="CASE SUM(CASE S_A.SOent_Q_CD WHEN "+prev_QCD+" THEN CASE WHEN S_A.SOent_Q_GB IN ("+prev_QGB+","+arrQGB1.get(arrQCD1.size()-1)+") THEN 1 ELSE 0 END ELSE 0 END) WHEN 2 THEN 1 WHEN 1 THEN 1 ELSE 0 END cnt_"+prev_QCD+prev_QGB;
		} else {
			if( !sSurveySQL.equals("") ) { sSurveySQL+=", \n"; }
			sSurveySQL+="SUM(CASE S_A.SOent_Q_CD WHEN "+prev_QCD+" THEN CASE S_A.SOent_Q_GB WHEN "+prev_QGB+" THEN 1 ELSE 0 END ELSE 0 END) cnt_"+prev_QCD+prev_QGB;
		}
		...
		for(int i=0; i<arrQCD2.size(); i++) {
			...
			if( tmp_Duble.equals("T") ) {
				if( !sSurveySQL.equals("") ) { sSurveySQL+=", \n"; }
				sSurveySQL+="CASE SUM(CASE S_A.SOent_Q_CD WHEN "+prev_QCD+" THEN CASE WHEN S_A.SOent_Q_GB IN ("+prev_QGB+","+arrQGB2.get(i)+") THEN 1 ELSE 0 END ELSE 0 END) WHEN 2 THEN 1 WHEN 1 THEN 1 ELSE 0 END cnt_"+prev_QCD+prev_QGB;
				tmp_Duble="F";
			} else if( nWriteCnt==2 ) {
				if( !sSurveySQL.equals("") ) { sSurveySQL+=", \n"; }
				sSurveySQL+="SUM(CASE S_A.SOent_Q_CD WHEN "+prev_QCD+" THEN CASE S_A.SOent_Q_GB WHEN "+prev_QGB+" THEN 1 ELSE 0 END ELSE 0 END) cnt_"+prev_QCD+prev_QGB;
		...
		if( tmp_Duble.equals("T") ) {
			if( !sSurveySQL.equals("") ) { sSurveySQL+=", \n"; }
			sSurveySQL+="CASE SUM(CASE S_A.SOent_Q_CD WHEN "+prev_QCD+" THEN CASE WHEN S_A.SOent_Q_GB IN ("+prev_QGB+","+arrQGB2.get(arrQCD2.size()-1)+") THEN 1 ELSE 0 END ELSE 0 END) WHEN 2 THEN 1 WHEN 1 THEN 1 ELSE 0 END cnt_"+prev_QCD+prev_QGB;
		} else {
			if( !sSurveySQL.equals("") ) { sSurveySQL+=", \n"; }
			sSurveySQL+="SUM(CASE S_A.SOent_Q_CD WHEN "+prev_QCD+" THEN CASE S_A.SOent_Q_GB WHEN "+prev_QGB+" THEN 1 ELSE 0 END ELSE 0 END) cnt_"+prev_QCD+prev_QGB;
		}
