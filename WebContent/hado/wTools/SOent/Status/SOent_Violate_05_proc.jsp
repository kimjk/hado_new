...
/*=================================== Record Selection Processing =====================================*/
	...
	try {
		...
		if( !tt.equals("start") ) {			
			...
			sSQLs+="SUM(CASE Index_Value WHEN '4' THEN 1 ELSE 0 END) d FROM ( \n";
			...
			if( currentYear==2010 ) {
				sSQLs+="		WHERE Sent_Q_CD=15 AND Sent_Q_GB=5 AND Current_Year='"+currentYear+"' \n";
			} else {
				sSQLs+="		WHERE Sent_Q_CD=13 AND Sent_Q_GB=5 AND Current_Year='"+currentYear+"' \n";
					}
...
content+="		<td><%if( arrData[ni][6] != null) {%><%=formater.format(Float.parseFloat(arrData[ni][5]) / Float.parseFloat(arrData[ni][2]) * 100F)%><%} else {%><%=0.0%><%}%>%</td>";
content+="	</tr>";
	<%}%>
<%}%>
