...
* 프로그램명	 : SOent_Violate_45_proc.jsp																				   
* 프로그램설명	: 수급사업자 > 조사결과분석 > 원재료 인상에 따른 하도급대금 인상요청 수용 여부 
* 프로그램버전	: 1.0.1																				                        
		...
		if( !tt.equals("start") ) {
			if (currentYear>=2010 ) { 
				if( !session.getAttribute("wgb").equals("") ) {
					sSQLs="SELECT a.Oent_Type,b.Common_NM,a.Field01,a.Field02,a.Field03,a.Field04,a.Field05,a.Field06,a.Field07 \n";
					sSQLs+="FROM (SELECT "+currentOent+".Oent_Type, \n";
				} else {
					sSQLs="SELECT Oent_GB Oent_Type,'' Common_NM,Field01,Field02,Field03,Field04,Field05,Field06,Field07 \n";
				...
				if( currentYear>=2012 ) {
					...
					sSQLs+="SUM(CASE HADO_TB_SOent_Answer.d WHEN '1' THEN 1 ELSE 0 END) Field05, \n";
					sSQLs+="SUM(CASE HADO_TB_SOent_Answer.e WHEN '1' THEN 1 ELSE 0 END) Field06, \n";
					sSQLs+="SUM(CASE HADO_TB_SOent_Answer.f WHEN '1' THEN 1 ELSE 0 END) Field07 \n";
				...
				*/
				...
				if( session.getAttribute("wgb").equals("1") ) {
					if( currentYear>=2012 ) {
						sSQLs+="AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=6 \n";
					} else {
						sSQLs+="AND HADO_TB_SOent_Answer.soent_q_cd=19 AND HADO_TB_SOent_Answer.soent_q_gb=6 \n";
					}
				} else if( session.getAttribute("wgb").equals("2") ) {
					if( currentYear>=2015 ) {
					sSQLs+="		AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=13 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=6 \n";
					} else {
						sSQLs+="AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=12 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=6 \n";
					}
				} else if( session.getAttribute("wgb").equals("3") ) {
					if( currentYear>=2012 ) {
						sSQLs+="AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=6 \n";
					} else {
						sSQLs+="AND HADO_TB_SOent_Answer.soent_q_cd=17 AND HADO_TB_SOent_Answer.soent_q_gb=6 \n";
					}
				} else {
					if( currentYear>=2015 ) {
					sSQLs+="		AND ( (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=6 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='1') \n";
					sSQLs+="			OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=13 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=6 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='2') \n";
					sSQLs+="			OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=6 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='3') ) \n";
					} else if( currentYear>=2012 ) {
						sSQLs+="AND ( (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=6 AND HADO_TB_Subcon_"+currentYear+".Oent_GB='1') \n";
						sSQLs+="	OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=12 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=6 AND HADO_TB_Subcon_"+currentYear+".Oent_GB='2') \n";
						sSQLs+="	OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=9 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=6 AND HADO_TB_Subcon_"+currentYear+".Oent_GB='3') ) \n";
					} else {
						sSQLs+="AND ((HADO_TB_SOent_Answer.soent_q_cd=19 AND HADO_TB_SOent_Answer.soent_q_gb=6  AND HADO_TB_Subcon_"+currentYear+".oent_gb='1') \n";
						sSQLs+="OR ( HADO_TB_SOent_Answer.soent_q_cd=17  AND  HADO_TB_SOent_Answer.soent_q_gb=6  AND HADO_TB_Subcon_"+currentYear+".oent_gb='3')) \n";
					}
				}
			...
			while( rs.next() ) {
				...
				arrData[nLoop][9]=StringUtil.checkNull(rs.getString("Common_NM")).trim();
...
content+="		<th colspan='6'>하도급대금 인상요청 수용 여부</th>";
content+="  </tr>";
content+="	<tr>";
content+="		<th>100%(전부 수용)</th>";
content+="		<th>75~100% 미만</th>";
content+="		<th>50~75% 미만</th>";
content+="		<th>25~50% 미만</th>";
content+="		<th>25% 미만</th>";
content+="		<th>전혀 수용해 주지 않았음</th>";
content+="  </tr>";
