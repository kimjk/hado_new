...
* 프로그램명	 : SOent_Violate_62proc.jsp																				   
* 프로그램설명	: 수급사업자 > 조사결과분석 > 하도급계약 체결 방법   
* 프로그램버전	: 1.0.1																				                        
	...
	try {
		...
		if( !tt.equals("start") ) {
			...
			if( !session.getAttribute("wgb").equals("") ) {
				...
				sSQLs="SELECT Oent_GB Oent_Type,'' Common_NM,Field01,Field02,Field03,Field04 FROM ( \n";
			...
			*/
			...
			if( session.getAttribute("wgb").equals("1") ) {
				sSQLs+="		AND HADO_TB_SOent_Answer_"+currentYear+".soent_q_cd=7 AND HADO_TB_SOent_Answer_"+currentYear+".soent_q_gb=1 \n";
			} else if( session.getAttribute("wgb").equals("2") ) {
				sSQLs+="		AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=7 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=1 \n";
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="		AND HADO_TB_SOent_Answer_"+currentYear+".soent_q_cd=7 AND HADO_TB_SOent_Answer_"+currentYear+".soent_q_gb=1 \n";
			} else {
				sSQLs+="		AND ( (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=7 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=1 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='1') \n";
				sSQLs+="			OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=7 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=1 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='2') \n";
				sSQLs+="			OR (HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_CD=7 AND HADO_TB_SOent_Answer_"+currentYear+".SOent_Q_GB=1 AND HADO_TB_SOent_Answer_"+currentYear+".Oent_GB='3') ) \n";
			}
...
content+="		<th colspan='3'>하도급계약 체결 방법</th>";
content+="  </tr>";
content+="	<tr>";
content+="		<th>전부 서면계약 (팩스, 전자문서 포함)</th>";
content+="		<th>전부 구두계약 (전화 포함)</th>";
content+="		<th>일부 서면계약, 일부 구두계약</th>";
content+="  </tr>";
