...
* 프로그램명	: Oent_Violate_04_Excel.jsp
* 프로그램설명	: 원사업자 > 조사결과분석 > 기술자료 제공 요구
* 프로그램버전	: 1.0.1
...
/*=================================== Record Selection Processing =====================================*/
	...
	try {
		...
		if( session.getAttribute("wgb").equals("1") ) {
			...
			} else if( currentYear == 2005 || currentYear == 2009 || currentYear == 2010 || currentYear == 2011) {
				sSQLs+="	AND Oent_GB='1' AND Oent_Q_CD=22 AND Oent_Q_GB=1 \n";
			} else {
				sSQLs+="	AND Oent_GB='1' AND Oent_Q_CD=23 AND Oent_Q_GB=1 \n";
			}
		} else if( session.getAttribute("wgb").equals("2") ) {
			if( currentYear >= 2014 ) {
				sSQLs+="	AND Oent_GB='2' AND Oent_Q_CD=13 AND Oent_Q_GB=1 \n";
			} else if( currentYear >= 2012 ) {
				sSQLs+="	AND Oent_GB='2' AND Oent_Q_CD=16 AND Oent_Q_GB=1 \n";
			} else {
				sSQLs+="	AND Oent_GB='2' AND Oent_Q_CD=25 AND Oent_Q_GB=1 \n";
			}
			...
			} else if( currentYear == 2009 || currentYear == 2010 ) {
				sSQLs+="	AND Oent_GB='3' AND Oent_Q_CD=21 AND Oent_Q_GB=1 \n";
			} else {
				sSQLs+="	AND Oent_GB='3' AND Oent_Q_CD=22 AND Oent_Q_GB=1 \n";
			}
		} else {
			if( currentYear >= 2014 ) {
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=13 AND Oent_Q_GB=1) \n";
				sSQLs+="	OR (Oent_GB='2' AND Oent_Q_CD=13 AND Oent_Q_GB=1) \n";
				...
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=22 AND Oent_Q_GB=1) \n";
				sSQLs+="	OR (Oent_GB='2' AND Oent_Q_CD=25 AND Oent_Q_GB=1) \n";
				sSQLs+="	OR (Oent_GB='3' AND Oent_Q_CD=21 AND Oent_Q_GB=1) ) \n";
			} else if( currentYear == 2005 || currentYear == 2009 ) {
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=22 AND Oent_Q_GB=1) \n";
				sSQLs+="	OR (Oent_GB='3' AND Oent_Q_CD=21 AND Oent_Q_GB=1) ) \n";
			} else {
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=23 AND Oent_Q_GB=1) \n";
				sSQLs+="	OR (Oent_GB='2' AND Oent_Q_CD=25 AND Oent_Q_GB=1) \n";
				sSQLs+="	OR (Oent_GB='3' AND Oent_Q_CD=22 AND Oent_Q_GB=1) ) \n";
			}
		}
