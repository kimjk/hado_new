...
* 프로그램명	: Oent_Violate_44_Excel.jsp
* 프로그램설명	: 원사업자 > 조사결과분석 > 기술자료 요청 수량
* 프로그램버전	: 1.0.1
	...
	try {
		...
		if( currentYear>=2015 ) {
			if( session.getAttribute("wgb").equals("1") ) {
				...
				sSQLs+="	AND Oent_GB='2' AND Oent_Q_CD=13 AND Oent_Q_GB=2 \n";
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="	AND Oent_GB='3' AND Oent_Q_CD=12 AND Oent_Q_GB=2 \n";
			} else {
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=13 AND Oent_Q_GB=2) \n";
				sSQLs+="		OR (Oent_GB='2' AND Oent_Q_CD=13 AND Oent_Q_GB=2 ) \n";
