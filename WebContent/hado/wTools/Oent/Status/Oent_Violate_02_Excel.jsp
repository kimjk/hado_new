...
* 프로그램명	: Oent_Violate_02_Excel.jsp
* 프로그램설명	: 원사업자 > 조사결과분석 > 하도급단가 인하율(응답 업체 기준)
* 프로그램버전	: 1.0.1
...
<html>
	...
	<body>
		...
		<%if( !stt.equals("start") ) {%>
			...
			<tr onMouseOver='tbRow_Over(this)' onMouseOut='tbRow_Out(this)'>
				<%if( !session.getAttribute("wgb").equals("") ) {%>
				<th><%=arrData[ni][8]%> (<%=arrData[ni][9]%>)</th>
				<%} else {%>
				...
				<%if( Integer.parseInt(arrData[ni][1]) > 0 && arrData[ni][5] != null) {%>
				...
				<%if( Integer.parseInt(arrData[ni][1]) > 0 && arrData[ni][6] != null) {%>
				...
				<%if( Integer.parseInt(arrData[ni][1]) > 0 && arrData[ni][7] != null) {%>
