...
* 프로그램명	: Oent_Violate_38_Excel.jsp
* 프로그램설명	: 원사업자 > 조사결과분석 > 물품(목적물)수령 요인(제조/용역)
* 프로그램버전	: 1.0.1
...
/*---------------------------------------- Variable Difinition ----------------------------------------*/
	...
	String[] arrItemText = new String[7];

	int nItemCount = 7; // 항목개수
	int nLoop = 1;
	int sStartYear = 2009;
...
/*=================================== Record Selection Processing =====================================*/
	...
	try {
		...
		sSQLs+="SUM(CASE C.G WHEN '1' THEN 1 ELSE 0 END) AS Field07 FROM ( \n";
		...
		if( currentYear>=2014 ) {
			if( session.getAttribute("wgb").equals("1") ) {
				sSQLs+="	AND Oent_GB='1' AND Oent_Q_CD=10 AND Oent_Q_GB=1 AND A='1' \n";
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="	AND Oent_GB='3' AND Oent_Q_CD=9 AND Oent_Q_GB=1 AND A='1' \n";
			} else {
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=10 AND Oent_Q_GB=1 AND A='1') \n";
				sSQLs+="		OR (Oent_GB='3' AND Oent_Q_CD=9 AND Oent_Q_GB=1 AND A='1') ) \n";
			}
		} else if( currentYear>=2012 ) {
			if( session.getAttribute("wgb").equals("1") ) {
		...
		if( currentYear>=2014 ) {
			if( session.getAttribute("wgb").equals("1") ) {
				sSQLs+="	AND Oent_GB='1' AND Oent_Q_CD=10 AND Oent_Q_GB=2 \n";
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="	AND Oent_GB='3' AND Oent_Q_CD=9 AND Oent_Q_GB=2 \n";
			} else {
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=10 AND Oent_Q_GB=2) \n";
				sSQLs+="		OR (Oent_GB='3' AND Oent_Q_CD=9 AND Oent_Q_GB=2) ) \n";
			}
		} else if( currentYear>=2012 ) {
			if( session.getAttribute("wgb").equals("1") ) {
		...
		while( rs.next() ) {
			...
			arrData[nLoop][10] = rs.getString("Common_NM")==null ? "":new String(rs.getString("Common_NM").trim().getBytes("ISO8859-1"), "EUC-KR" );
	...
	// Default Value
	arrItemText[0] = "납품한 제품이 불량품";
	arrItemText[1] = "발주서대로 제조되지 않음";
	arrItemText[2] = "발주번경 또는 취소";
	arrItemText[3] = "수급사업자의 책임";
	arrItemText[4] = "고객의 클레임, 판매부진";
	arrItemText[5] = "사양변경, 모델단종";
	arrItemText[6] = "기타";
...
<html>
	...
	<body>
		...
		<%if( !stt.equals("start") ) {%>
			...
			<tr onMouseOver='tbRow_Over(this)' onMouseOut='tbRow_Out(this)'>
				<%if( !session.getAttribute("wgb").equals("") ) {%>
				<th rowspan='2'><%=arrData[ni][9]%> (<%=arrData[ni][10]%>)</th>
				<%} else {%>
