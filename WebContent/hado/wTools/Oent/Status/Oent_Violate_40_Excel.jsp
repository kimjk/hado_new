...
* 프로그램명	: Oent_Violate_40_Excel.jsp
* 프로그램설명	: 원사업자 > 조사결과분석 > 위탁의 취소ㆍ변경 요인 
* 프로그램버전	: 1.0.1
...
/*---------------------------------------- Variable Difinition ----------------------------------------*/
	...
	String[][] arrData = new String[51][10];
	float[] arrSum = new float[15];

	// 2010년 답변항목 내용 변경에 따른 예외처리를 위한 배열 선언 / 20100716 / 정광식
	String[] arrItemText = new String[8];

	int nItemCount = 6; // 항목개수
	int nLoop = 1;
	int sStartYear = 2009;
...
/*=================================== Record Selection Processing =====================================*/
	...
	try {
		...
		if( currentYear>=2014 ) {
			if( session.getAttribute("wgb").equals("1") ) {
				...
				sSQLs+="	AND Oent_GB='2' AND Oent_Q_CD=11 AND Oent_Q_GB=1 AND A='1' \n";
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="	AND Oent_GB='3' AND Oent_Q_CD=10 AND Oent_Q_GB=1 AND A='1' \n";
			} else {
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=11 AND Oent_Q_GB=1 AND A='1') \n";
				sSQLs+="		OR (Oent_GB='2' AND Oent_Q_CD=11 AND Oent_Q_GB=1 AND A='1' ) \n";
			...
			if( session.getAttribute("wgb").equals("1") ) {
				...
				sSQLs+="	AND Oent_GB='2' AND Oent_Q_CD=14 AND Oent_Q_GB=1 AND A='1' \n";
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="	AND Oent_GB='3' AND Oent_Q_CD=12 AND Oent_Q_GB=1 AND A='1' \n";
			} else {
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=13 AND Oent_Q_GB=1 AND A='1') \n";
				sSQLs+="		OR (Oent_GB='2' AND Oent_Q_CD=14 AND Oent_Q_GB=1 AND A='1' ) \n";
				sSQLs+="		OR (Oent_GB='3' AND Oent_Q_CD=12 AND Oent_Q_GB=1 AND A='1') ) \n";
			}
		} else {
			if( session.getAttribute("wgb").equals("1") ) {
				sSQLs+="	AND Oent_GB='1' AND Oent_Q_CD=13 AND Oent_Q_GB=3 AND A='1' \n";
			} else if( session.getAttribute("wgb").equals("2") ) {
				sSQLs+="	AND Oent_GB='2' AND Oent_Q_CD=14 AND Oent_Q_GB=3 AND A='1' \n";
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="	AND Oent_GB='3' AND Oent_Q_CD=12 AND Oent_Q_GB=3 AND A='1' \n";
			} else {
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=13 AND Oent_Q_GB=3 AND A='1') \n";
				sSQLs+="		OR (Oent_GB='2' AND Oent_Q_CD=14 AND Oent_Q_GB=3 AND A='1' )  \n";
				sSQLs+="		OR (Oent_GB='3' AND Oent_Q_CD=12 AND Oent_Q_GB=3 AND A='1') )  \n";
			}
		}	
		...
		if( currentYear>=2014 ) {
			if( session.getAttribute("wgb").equals("1") ) {
				...
				sSQLs+="	AND Oent_GB='2' AND Oent_Q_CD=11 AND Oent_Q_GB=2 \n";
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="	AND Oent_GB='3' AND Oent_Q_CD=10 AND Oent_Q_GB=2 \n";
			} else {
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=11 AND Oent_Q_GB=2) \n";
				sSQLs+="		OR (Oent_GB='2' AND Oent_Q_CD=11 AND Oent_Q_GB=2 ) \n";
				sSQLs+="		OR (Oent_GB='3' AND Oent_Q_CD=10 AND Oent_Q_GB=2) ) \n";
			}
		} else if( currentYear>=2012 ) {
			if( session.getAttribute("wgb").equals("1") ) {
				sSQLs+="	AND Oent_GB='1' AND Oent_Q_CD=13 AND Oent_Q_GB=2 \n";
			} else if( session.getAttribute("wgb").equals("2") ) {
				sSQLs+="	AND Oent_GB='2' AND Oent_Q_CD=14 AND Oent_Q_GB=2 \n";
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="	AND Oent_GB='3' AND Oent_Q_CD=12 AND Oent_Q_GB=2 \n";
			} else {
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=13 AND Oent_Q_GB=2) \n";
				sSQLs+="		OR (Oent_GB='2' AND Oent_Q_CD=14 AND Oent_Q_GB=2 ) \n";
				sSQLs+="		OR (Oent_GB='3' AND Oent_Q_CD=12 AND Oent_Q_GB=2) ) \n";
			}
		} else {
			if( session.getAttribute("wgb").equals("1") ) {
				sSQLs+="	AND Oent_GB='1' AND Oent_Q_CD=13 AND Oent_Q_GB=4 \n";
			} else if( session.getAttribute("wgb").equals("2") ) {
				sSQLs+="	AND Oent_GB='2' AND Oent_Q_CD=14 AND Oent_Q_GB=4 \n";
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="	AND Oent_GB='3' AND Oent_Q_CD=12 AND Oent_Q_GB=4 \n";
			} else {
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=13 AND Oent_Q_GB=4) \n";
				sSQLs+="		OR (Oent_GB='2' AND Oent_Q_CD=14 AND Oent_Q_GB=4)  \n";
	...
	// Default Value
	arrItemText[0] = "발주자의 주문취소";
	arrItemText[1] = "수급사업자의 부도로 정산합의";
	arrItemText[2] = "판매부진";
	arrItemText[3] = "사양변경, 모델단종";
	arrItemText[4] = "수급사업자의 계약불이행";
	arrItemText[5] = "기타";

	 if( session.getAttribute("wgb").equals("2") ) {
		arrItemText[0] = "발주자의 발주취소";
		arrItemText[1] = "수급사업자의 부도";
		arrItemText[2] = "수급사업자의 시공능력 부족";
		arrItemText[3] = "회사경영 악화";
		arrItemText[4] = "계약금액보다 양호한 조건의 금액을 제시한 업체 존재";
		arrItemText[5] = "수급사업자의 계약불이행";
		nItemCount = 6;
	} else if( session.getAttribute("wgb").equals("3") ) {
		arrItemText[0] = "발주자의 주문취소";
		arrItemText[1] = "판매 또는 흥행실패가 예상됨";
		arrItemText[2] = "수급사업자의 계약불이행";
		arrItemText[3] = "수급사업자의 부도로 정산합의";
		arrItemText[4] = "기타";
		nItemCount = 5;
	}
/*=====================================================================================================*/
...
<html>
	...
	<body>
		...
		<%if( !stt.equals("start") ) {%>
			...
			<tr onMouseOver='tbRow_Over(this)' onMouseOut='tbRow_Out(this)'>
				<%if( !session.getAttribute("wgb").equals("") ) {%>
				<th rowspan='2'><%=arrData[ni][8]%> (<%=arrData[ni][9]%>)</th>
				<%} else {%>
