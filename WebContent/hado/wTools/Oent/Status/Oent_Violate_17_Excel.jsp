...
* 프로그램명	: Oent_Violate_17_Excel.jsp
* 프로그램설명	: 원사업자 > 조사결과분석 > 표준하도급계약서 미사용 사유
* 프로그램버전	: 1.0.1
...
/*=================================== Record Selection Processing =====================================*/
	...
	try {
		...
		if( session.getAttribute("wgb").equals("1") ) {
			if( currentYear >= 2014) {
				sSQLs+="			AND Oent_Q_CD=15 AND Oent_Q_GB=2  AND Oent_GB='1' \n";
			} else if( currentYear == 2003) {
				sSQLs+="			AND Oent_Q_CD=9 AND Oent_Q_GB=2  AND Oent_GB='1' \n";
			} else if( currentYear <= 2002) {
				sSQLs+="			AND Oent_Q_CD=3 AND Oent_Q_GB=2  AND Oent_GB='1' \n";
			} else {
			...
			if( currentYear >= 2015) {
				...
				sSQLs+="			AND Oent_Q_CD=15 AND Oent_Q_GB=2  AND Oent_GB='2' \n";
			} else if( currentYear == 2010) {
				sSQLs+="			AND Oent_Q_CD=11 AND Oent_Q_GB=2  AND Oent_GB='2' \n";
			} else if( currentYear == 2003) {
				sSQLs+="			AND Oent_Q_CD=9 AND Oent_Q_GB=2  AND Oent_GB='2' \n";
			} else if( currentYear <= 2002) {
				sSQLs+="			AND Oent_Q_CD=3 AND Oent_Q_GB=2  AND Oent_GB='2' \n";
			} else {
			...
			if( currentYear >= 2014 ) {
				sSQLs+="			AND Oent_Q_CD=14 AND Oent_Q_GB=2  AND Oent_GB='3' \n";
			} else if( currentYear == 2012 ) {
				sSQLs+="			AND Oent_Q_CD=9 AND Oent_Q_GB=2  AND Oent_GB='3' \n";
			} else {
			...
			if( currentYear >= 2015 ) {
				...
				sSQLs+="				OR (Oent_Q_CD=14 AND Oent_Q_GB=2  AND Oent_GB='3') ) \n";
			} else if( currentYear >= 2014 ) {
				sSQLs+="			AND ( (Oent_Q_CD=15 AND Oent_Q_GB=2  AND Oent_GB='1') \n";
				sSQLs+="				OR (Oent_Q_CD=15 AND Oent_Q_GB=2  AND Oent_GB='2') \n";
				sSQLs+="				OR (Oent_Q_CD=14 AND Oent_Q_GB=2  AND Oent_GB='3') ) \n";
			} else if( currentYear == 2012 ) {
				sSQLs+="			AND ( (Oent_Q_CD=10 AND Oent_Q_GB=2  AND Oent_GB='1') \n";
				sSQLs+="				OR (Oent_Q_CD=10 AND Oent_Q_GB=2  AND Oent_GB='2') \n";
				sSQLs+="				OR (Oent_Q_CD=9 AND Oent_Q_GB=2  AND Oent_GB='3') ) \n";
			} else if( currentYear == 2010 ) {
				sSQLs+="			AND ( (Oent_Q_CD=10 AND Oent_Q_GB=2  AND Oent_GB='1') \n";
				sSQLs+="				OR (Oent_Q_CD=11 AND Oent_Q_GB=2  AND Oent_GB='2') \n";
				...
				sSQLs+="			AND ( (Oent_Q_CD=3 AND Oent_Q_GB=2 AND Oent_GB='1') \n";
				sSQLs+="				OR (Oent_Q_CD=3 AND Oent_Q_GB=2  AND Oent_GB='2') \n";
				sSQLs+="				OR (Oent_Q_CD=3 AND Oent_Q_GB=2  AND Oent_GB='3') )  \n";
			} else {
				sSQLs+="			AND ( (Oent_Q_CD=10 AND Oent_Q_GB=2 AND Oent_GB='1') \n";
				sSQLs+="				OR (Oent_Q_CD=10 AND Oent_Q_GB=2  AND Oent_GB='2') \n";
				sSQLs+="				OR (Oent_Q_CD=10 AND Oent_Q_GB=2  AND Oent_GB='3') )  \n";
	...
	<body>
		...
		<table border='1'>
			<tr>
				<th rowspan='2'>구분</th>
				<th rowspan='2'>응답수</th>
				<th colspan='5'>미사용 사유</th>
			</tr>
			<tr>
				<th>당사가 필요성을 느끼지 못했기 때문</th>
				<th>당해 업종의 표준하도급계약서가 존재하지 않기 때문</th>
				<th>수급사업자가 원하지 않았기 때문</th>
				<th>현실과 맞지 않기 때문</th>
				<th>몰랐기 때문</th>
			</tr>
