	...
	try {
		...
		if( currentYear >= 2009 ) {
			...
			if( session.getAttribute("wgb").equals("1") ) {
				sSQLs+="			AND Oent_Q_CD=22 AND Oent_Q_GB=4 AND A='1' AND Oent_GB='1' \n";	
			} else if( session.getAttribute("wgb").equals("2") ) {
				sSQLs+="			AND Oent_Q_CD=25 AND Oent_Q_GB=4 AND A='1' AND Oent_GB='2' \n";	
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="			AND Oent_Q_CD=21 AND Oent_Q_GB=4 AND A='1' \n";	
			} else {
				sSQLs+="			AND ( (Oent_Q_CD=22 AND Oent_Q_GB=4 AND A='1' AND Oent_GB='1') \n";	
				sSQLs+="				OR (Oent_Q_CD=25 AND Oent_Q_GB=4 AND A='1' AND Oent_GB='2') \n";
				sSQLs+="				OR (Oent_Q_CD=21 AND Oent_Q_GB=4 AND A='1' AND Oent_GB='3') ) \n";	
			}
			...
			// 업종별 "영업비밀ㆍ기술자료요구에 따른 분쟁" 해당 문항 번호
			if( session.getAttribute("wgb").equals("1") ) {
				sSQLs+="			AND Oent_Q_CD=22 AND Oent_Q_GB=6 AND Oent_GB='1' \n";
			} else if( session.getAttribute("wgb").equals("2") ) {
				sSQLs+="			AND Oent_Q_CD=25 AND Oent_Q_GB=6 AND Oent_GB='2' \n";
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="			AND Oent_Q_CD=21 AND Oent_Q_GB=6 AND Oent_GB='3' \n";
			} else {
				sSQLs+="			AND ( (Oent_Q_CD=22 AND Oent_Q_GB=6 AND Oent_GB='1') \n";
				sSQLs+="				OR (Oent_Q_CD=25 AND Oent_Q_GB=6 AND Oent_GB='2') \n";
				sSQLs+="				OR (Oent_Q_CD=21 AND Oent_Q_GB=6 AND Oent_GB='3') ) \n";
			}
			...
			if( session.getAttribute("wgb").equals("1") ) {
				if( currentYear != 2006 && currentYear!=2007 && currentYear!=2008 ) {
					sSQLs+="			AND Oent_Q_CD=22 AND Oent_Q_GB=6  AND Oent_GB='1' \n";
				} else {
					sSQLs+="			AND Oent_Q_CD=23 AND Oent_Q_GB=6  AND Oent_GB='1' \n";
				}
				...
				if( currentYear != 2006 && currentYear!=2007 && currentYear!=2008 ) {
					...
					sSQLs+="			AND Oent_Q_CD=22 AND Oent_Q_GB=6  AND Oent_GB='3' \n";
				}
			} else {
				if( currentYear != 2006 && currentYear!=2007 && currentYear!=2008 ) {
					sSQLs+="			AND ( (Oent_Q_CD=22 AND Oent_Q_GB=6 AND Oent_GB='1') \n";
					...
					sSQLs+="			AND ( (Oent_Q_CD=23 AND Oent_Q_GB=6 AND Oent_GB='1') \n";
					sSQLs+="				OR (Oent_Q_CD=22 AND Oent_Q_GB=6  AND Oent_GB='3') )  \n";
				}
			}
			sSQLs+="				AND Comp_Status='1' \n";
			sSQLs+="				AND Oent_Status='1' \n";
...
content+="		<th colspan='2'>확대</th>";
content+="		<th colspan='2'>동일</th>";
content+="		<th colspan='2'>축소</th>";
content+="	</tr>";
content+="	<tr>";
