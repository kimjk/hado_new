...
* 프로그램명	: Oent_Violate_48_proc.jsp
...
/*=================================== Record Selection Processing =====================================*/
	...
	try {
		...
		if( currentYear>=2014 ) {
			...
			if( session.getAttribute("wgb").equals("1") ) {
				...
				sSQLs+="	AND Oent_GB='2' AND Oent_Q_CD=9 AND Oent_Q_GB=2 \n";	
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="	AND Oent_GB='3' AND Oent_Q_CD=8 AND Oent_Q_GB=2 \n";	
			} else {
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=9 AND Oent_Q_GB=2) \n";
				sSQLs+="		OR (Oent_GB='2' AND Oent_Q_CD=9 AND Oent_Q_GB=2 ) \n";
				sSQLs+="		OR (Oent_GB='3' AND Oent_Q_CD=8 AND Oent_Q_GB=2) ) \n";
			}
		} else {
			if( session.getAttribute("wgb").equals("1") ) {
				sSQLs+="	AND Oent_GB='1' AND Oent_Q_CD=8 AND Oent_Q_GB=1 \n";
			} else if( session.getAttribute("wgb").equals("2") ) {
				sSQLs+="	AND Oent_GB='2' AND Oent_Q_CD=9 AND Oent_Q_GB=1 \n";
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="	AND Oent_GB='3' AND Oent_Q_CD=8 AND Oent_Q_GB=1 \n";
			} else {
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=8 AND Oent_Q_GB=1) \n";
				sSQLs+="		OR (Oent_GB='2' AND Oent_Q_CD=9 AND Oent_Q_GB=1 )  \n";
				sSQLs+="		OR (Oent_GB='3' AND Oent_Q_CD=8 AND Oent_Q_GB=1) )  \n";
			}
		}	
	...
	// Default Value
	if( currentYear>=2015 ) {
	arrItemText[0] = "모든 하도급계약에 교부함";
	arrItemText[1] = "일부 하도급계약에 교부함";
	arrItemText[2] = "모든 하도급계약에 교부하지 않음";
	} else {
	arrItemText[0] = "전부 서면계약(팩스, 전자문서 포함)";
	arrItemText[1] = "전부 구두계약(전화 포함)";
	arrItemText[2] = "일부 서면계약, 일부 구두계약";
	}
...
content+="	<tr>";
content+="		<th><a href='Oent_Violate_48_Excel.jsp' target='_blank'><img src='/hado/hado/wTools/img/img_forExcel.gif'></a></th>";
content+="	</tr>";
content+="</table>";
