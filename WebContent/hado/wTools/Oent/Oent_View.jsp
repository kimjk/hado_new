			...
			*/
			...
			if(sCYearNum == 2021) {
				...
				while ( rs.next() ) {
					...
					c = rs.getString("C")==null ? "0":rs.getString("C");
				}
				rs.close();
...
public String statusf_2020(String str){
  String sReturnStr = "";
...
//<![CDATA[
	...
	content+="	<tr>";
	content+="		<th>설립연월</th>";
	content+="		<td>&nbsp;<%=sOentIncorp1%>년 <%=sOentIncorp2%>월</td>";
	...
	content+="		<td>&nbsp;<%=statusf_2021(compstatus)%></td>";
	<%
	} else if( sCYearNum == 2020 ) {
	%>
	content+="		<th>회사영업상태</th>";
	content+="		<td>&nbsp;<%=statusf_2020(compstatus)%></td>";
	<%
	} else {
	%>
	content+="		<th>회사영업상태</th>";
	...
	content+="<div id='viewWinTitle'>회사 일반현황 비정상영업 사업자 증빙자료</div><a href=javascript:top.f_attachFileDel('<%=sMngNo%>','<%=sCYear%>','<%=sOentGB%>') class='sbutton'>증빙자료초기화</a>";
	...
	content+="			<li class='fl'><a href=javascript:top.f_InfoDel('<%=sMngNo%>','<%=sCYear%>','<%=sOentGB%>') class='sbutton'>회사개요초기화</a></li>";
