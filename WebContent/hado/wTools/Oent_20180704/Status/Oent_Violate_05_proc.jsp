...
/*=================================== Record Selection Processing =====================================*/
	...
	try {
		...
		if( currentYear >= 2009 ) {
			...
			sSQLs+="	SUM(CASE C.B WHEN '1' THEN 1 ELSE 0 END) AS Field02 FROM ( \n";
			sSQLs+="		SELECT * FROM HADO_VT_Oent_Answer_TradeST A \n";
			sSQLs+="		WHERE Current_Year='"+currentYear+"'  \n";
			// 업종별 해당문항의 연계항목
			if( session.getAttribute("wgb").equals("1") ) {
				sSQLs+="			AND Oent_Q_CD=22 AND Oent_Q_GB=1 AND A='1' AND Oent_GB='1' \n";	
			} else if( session.getAttribute("wgb").equals("2") ) {
				sSQLs+="			AND Oent_Q_CD=25 AND Oent_Q_GB=1 AND A='1' AND Oent_GB='2' \n";	
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="			AND Oent_Q_CD=21 AND Oent_Q_GB=1 AND A='1' AND Oent_GB='3' \n";	
			} else {
				sSQLs+="			AND ( (Oent_Q_CD=22 AND Oent_Q_GB=1 AND A='1' AND Oent_GB='1') \n";	
				sSQLs+="				OR (Oent_Q_CD=25 AND Oent_Q_GB=1 AND A='1' AND Oent_GB='2') \n";
				sSQLs+="				OR (Oent_Q_CD=21 AND Oent_Q_GB=1 AND A='1' AND Oent_GB='3') ) \n";	
			}
			...
			if( session.getAttribute("wgb").equals("1") ) {
				sSQLs+="			AND Oent_Q_CD=22 AND Oent_Q_GB=2 AND Oent_GB='1' \n";
			} else if( session.getAttribute("wgb").equals("2") ) {
				sSQLs+="			AND Oent_Q_CD=25 AND Oent_Q_GB=2 AND Oent_GB='2' \n";
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="			AND Oent_Q_CD=21 AND Oent_Q_GB=2 AND Oent_GB='3' \n";
			} else {
				sSQLs+="			AND ( (Oent_Q_CD=22 AND Oent_Q_GB=2 AND Oent_GB='1') \n";
				sSQLs+="				OR (Oent_Q_CD=25 AND Oent_Q_GB=2 AND Oent_GB='2') \n";
				sSQLs+="				OR (Oent_Q_CD=21 AND Oent_Q_GB=2 AND Oent_GB='3') ) \n";
			}
		...
		while( rs.next() ) {
			...
			arrData[nLoop][5] = new String( StringUtil.checkNull(rs.getString("Common_NM")).trim().getBytes("ISO8859-1"), "EUC-KR" );
