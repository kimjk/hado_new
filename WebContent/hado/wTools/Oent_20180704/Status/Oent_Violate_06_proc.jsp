...
/*=================================== Record Selection Processing =====================================*/
	...
	try {
		...
		if( session.getAttribute("wgb").equals("1") ) {
			if( currentYear >= 2006 && currentYear <= 2008 ) {
				sSQLs+="			AND Oent_Q_CD=23 AND Oent_Q_GB=3  AND Oent_GB='1' \n";
			} else {
				sSQLs+="			AND Oent_Q_CD=22 AND Oent_Q_GB=3  AND Oent_GB='1' \n";
			}
			
		} else if( session.getAttribute("wgb").equals("2") ) {
			if( currentYear == 2010 ) {
				sSQLs+="			AND Oent_Q_CD=25 AND Oent_Q_GB=3  AND Oent_GB='2' \n";
			} 
		} else if( session.getAttribute("wgb").equals("3") ) {
			if( currentYear >= 2006 && currentYear <= 2008 ) {
				sSQLs+="			AND Oent_Q_CD=22 AND Oent_Q_GB=3  AND Oent_GB='3' \n";
			} else {
				sSQLs+="			AND Oent_Q_CD=21 AND Oent_Q_GB=3  AND Oent_GB='3' \n";
			}
		} else {
			if( currentYear == 2010 ) {
				sSQLs+="			AND ( (Oent_Q_CD=22 AND Oent_Q_GB=3  AND Oent_GB='1') \n";
				sSQLs+="				OR (Oent_Q_CD=25 AND Oent_Q_GB=3  AND Oent_GB='2') \n";
				sSQLs+="				OR (Oent_Q_CD=21 AND Oent_Q_GB=3  AND Oent_GB='3') ) \n";
			} else if( currentYear >= 2006 && currentYear <= 2008 ) {
				sSQLs+="			AND ( (Oent_Q_CD=23 AND Oent_Q_GB=3 AND Oent_GB='1') \n";
				sSQLs+="				OR (Oent_Q_CD=22 AND Oent_Q_GB=3  AND Oent_GB='3') )  \n";
			} else {
				sSQLs+="			AND ( (Oent_Q_CD=22 AND Oent_Q_GB=3 AND Oent_GB='1') \n";
				sSQLs+="				OR (Oent_Q_CD=21 AND Oent_Q_GB=3  AND Oent_GB='3') )  \n";
			}
