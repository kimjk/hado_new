...
* 프로그램명	: Oent_Violate_53_proc.jsp
* 프로그램설명	: 원사업자 > 조사결과분석 > 부당특약 여부
* 프로그램버전	: 1.0.1
	...
	try {
		...
		if( currentYear>=2015 ) {
			if( session.getAttribute("wgb").equals("1") ) {
				...
				sSQLs+="	AND Oent_GB='2' AND Oent_Q_CD=17 AND Oent_Q_GB=1 \n";	
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="	AND Oent_GB='3' AND Oent_Q_CD=15 AND Oent_Q_GB=1 \n";	
			} else {
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=16 AND Oent_Q_GB=1) \n";
				sSQLs+="		OR (Oent_GB='2' AND Oent_Q_CD=17 AND Oent_Q_GB=1 ) \n";
...
content+="	<tr>";
content+="		<th><a href='Oent_Violate_53_Excel.jsp' target='_blank'><img src='/hado/hado/wTools/img/img_forExcel.gif'></a></th>";
content+="	</tr>";
content+="</table>";
...
content+="		<th colspan='2'>일부 하도급계약에 있었음</th>";
