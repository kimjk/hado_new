...
* 프로그램명	: Oent_Violate_41_proc.jsp
* 프로그램설명	: 원사업자 > 조사결과분석 > 하도급대금 감액 여부
* 프로그램버전	: 1.0.1
	...
	try {
		...
		if( currentYear>=2014 ) {
			if( session.getAttribute("wgb").equals("1") ) {
				sSQLs+="	AND Oent_GB='1' AND Oent_Q_CD=12 AND Oent_Q_GB=1 \n";	
			} else if( session.getAttribute("wgb").equals("2") ) {
				sSQLs+="	AND Oent_GB='2' AND Oent_Q_CD=12 AND Oent_Q_GB=1 \n";	
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="	AND Oent_GB='3' AND Oent_Q_CD=11 AND Oent_Q_GB=1 \n";	
			} else {
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=12 AND Oent_Q_GB=1) \n";
				sSQLs+="		OR (Oent_GB='2' AND Oent_Q_CD=12 AND Oent_Q_GB=1 ) \n";
				sSQLs+="		OR (Oent_GB='3' AND Oent_Q_CD=11 AND Oent_Q_GB=1) ) \n";
			}
		} else if( currentYear>=2012 ) {
			if( session.getAttribute("wgb").equals("1") ) {
				sSQLs+="	AND Oent_GB='1' AND Oent_Q_CD=14 AND Oent_Q_GB=1 \n";	
			} else if( session.getAttribute("wgb").equals("2") ) {
				sSQLs+="	AND Oent_GB='2' AND Oent_Q_CD=15 AND Oent_Q_GB=1 \n";	
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="	AND Oent_GB='3' AND Oent_Q_CD=13 AND Oent_Q_GB=1 \n";	
			} else {
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=14 AND Oent_Q_GB=1) \n";
				sSQLs+="		OR (Oent_GB='2' AND Oent_Q_CD=15 AND Oent_Q_GB=1 ) \n";
				sSQLs+="		OR (Oent_GB='3' AND Oent_Q_CD=13 AND Oent_Q_GB=1) ) \n";
			}
		} else {
			if( session.getAttribute("wgb").equals("1") ) {
				sSQLs+="	AND Oent_GB='1' AND Oent_Q_CD=15 AND Oent_Q_GB=1 \n";
			} else if( session.getAttribute("wgb").equals("2") ) {
				sSQLs+="	AND Oent_GB='2' AND Oent_Q_CD=15 AND Oent_Q_GB=1 \n";
			} else if( session.getAttribute("wgb").equals("3") ) {
				sSQLs+="	AND Oent_GB='3' AND Oent_Q_CD=14 AND Oent_Q_GB=1 \n";
			} else {
				sSQLs+="	AND ( (Oent_GB='1' AND Oent_Q_CD=15 AND Oent_Q_GB=1) \n";
				sSQLs+="		OR (Oent_GB='2' AND Oent_Q_CD=15 AND Oent_Q_GB=1 )  \n";
				sSQLs+="		OR (Oent_GB='3' AND Oent_Q_CD=14 AND Oent_Q_GB=1) )  \n";
			}
		}	
...
content+="	<tr>";
content+="		<th><a href='Oent_Violate_41_Excel.jsp' target='_blank'><img src='/hado/hado/wTools/img/img_forExcel.gif'></a></th>";
content+="	</tr>";
content+="</table>";
...
content+="		<th colspan='2'>있었음</th>";
content+="		<th colspan='2'>없었음</th>";
content+="	</tr>";
content+="	<tr>";
...
content+="	<tr onMouseOver='tbRow_Over(this)' onMouseOut='tbRow_Out(this)'>";
		<%if( !session.getAttribute("wgb").equals("") ) {%>	
content+="		<th><%=arrData[ni][4]%> (<%=arrData[ni][5]%>)</th>";
		<%} else {%>
